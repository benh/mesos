  ...
  if (response.type == Response::PATH) {
    ...
    if (fd.isError()) {
      ...
      if (size.isError()) {
        ...
        send(new HttpResponseEncoder(response, request));

        // Note the file descriptor gets closed by FileEncoder.
        send(new FileEncoder(fd.get(), size->bytes()), request.keepAlive);
      }
    }
    ...
    const Future<string>& chunk)
{
  ...
  if (chunk.isReady()) {
    ...
    send(new DataEncoder(out.str()), finished ? request->keepAlive : true);
...
void HttpProxy::send(Encoder* encoder, bool persist)
{
  outgoing.put(Owned<Encoder>(encoder));
  if (!persist) {
    outgoing.put(None());
  }
}
...
void HttpProxy::send(const Response& response, const Request& request)
{
  ...
  send(new HttpResponseEncoder(response, request), persist);
}
