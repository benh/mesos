...
void ProcessManager::resume(ProcessBase* process)
{
  ...
  CHECK(state == ProcessBase::BOTTOM || state == ProcessBase::READY);

  if (state == ProcessBase::BOTTOM) {
  ...
  // than we'll never actually cleanup! This bug has been here a long
  // time!!!
...
bool ProcessManager::wait(const UPID& pid)
{
  ...
  if (ProcessReference reference = use(pid)) {
    ...
    switch (process->state.load()) {
      case ProcessBase::BOTTOM:
      case ProcessBase::READY:
...
void ProcessBase::enqueue(Event* event)
{
  ...
  // enqueuing us in the run queue only to find to find out in
  // `ProcessManager::resume` that our event queue is empty!
  old = state.load();
  if (old == BLOCKED && state.compare_exchange_strong(old, READY)) {
