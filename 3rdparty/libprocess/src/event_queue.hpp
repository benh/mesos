...
private:
  ...
  Event* dequeue()
  {
    // NOTE: for performance reasons we don't check `decomissioned`
    // here so it's possible that we'll loop forever if a consumer
    // called `decomission()` and then subsequently called
    // `dequeue()`.
    Event* event = nullptr;
    do {
  ...
  {
    // NOTE: for performance reasons we don't check `decomissioned`
    // here so it's possible that we'll return true when in fact we've
    // been decomissioned and you shouldn't attempt to dequeue
    // anything.
  ...
  size_t count()
  {
    ...
    queue.try_dequeue_bulk(std::back_inserter(items), 256);
  ...
  Event* try_dequeue()
  {
    ...
    do {
      ...
      // out of order event!
    } while (queue.try_dequeue_bulk(std::back_inserter(items), 32) != 0);
  ...
  // 18,446,744,073,709,551,615 nanoseconds == ~585 years! ;-)
  ...
  // consumer is ever reading or writing!
  uint64_t next = 0;
  ...
  std::atomic<bool> decomissioned = ATOMIC_VAR_INIT(false);
#endif // LOCK_FREE_EVENT_QUEUE
};
